...
| **Risk-label ritual** — re-asking for decisions already explicit, including a side effect jointly made explicit by a user's execution request and the named skill's purpose, or refusing a fully authorized task merely because its category is high risk | 1 Scale By Risk; 3 Handle Material Uncertainty |
...
| **Dropped requirements** — reporting done while parts of the request, a mixed task's applicable execution portion, or a focused workflow's primary completion criterion were silently dropped or reinterpreted | Applicability; Composition; 8 Report Only What Matters |
| **Process noise** — requiring an up-front state classification before any work, or applying execution ritual outside the protocol's scope; narrating irrelevant process; overstating confidence; or compressing a required focused artifact in the name of brevity | Applicability; Composition; 8 Report Only What Matters |
| **Workflow recursion** — treating a skill under edit as active instructions, or requiring another skill when this one does not apply, creates unrelated work or conflicts with a user's skill opt-out | Applicability; Composition |
